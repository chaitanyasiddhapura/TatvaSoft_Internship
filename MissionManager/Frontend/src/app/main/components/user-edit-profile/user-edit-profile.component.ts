...
export class UserEditProfileComponent implements OnInit, OnDestroy {
    ...
    const addUserSkillSubscribe = this._commonService
      ...
      .subscribe(
        (data: any) => {
          ...
          // ✅ Update form with selected skills
          this.userProfileForm.patchValue({
            mySkills: this.skillList1,
          });
 ...
 getCityList(event: Event) {
  ...
  closeAddYourSkillModal() {
    ...
    // ✅ Update form when modal is closed (just in case)
    this.userProfileForm.patchValue({ mySkills: this.skillList1 });
  }
